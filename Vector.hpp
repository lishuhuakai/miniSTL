...
#define DEFAULT_CAPACITY 3
	...
	protected:
		...
		Rank selectMax(Rank lo, Rank hi);			// 在向量下标为[lo, hi)的元素中中选取最大的下标
	...
	public:
		...
		{
			if (size_ == 0)
				throw UnderflowError("Vector popBack(): Vector is empty")
			size_--;
		}
	...
	// bubbleSort 冒泡排序, 对向量区间[lo, hi) 进行排序
	template <typename T>
	...
	void Vector<T>::merge(Rank lo, Rank mi, Rank hi) 
	{
		T* A = elem_ + lo; 
		int lb = mi - lo; 
		T* B = new T[lb]; // 前子向量B[0, lb) = elem_[lo, mi)
		for (Rank i = 0; i < lb; B[i] = A[i++]);
		int lc = hi - mi; 
		T* C = elem_ + mi; // 后子向量C[0, lc) = elem_[mi, hi)
		// 这里的归并,无非就是选择两个中较小的
	...
	// selectMax 选取下标向量下标[lo, hi)中最大的元素的下标
	template <typename T>
	Rank Vector<T>::selectMax(Rank lo, Rank hi)
	{
		...
		{
			if (elem_[i] > elem_[idx])
				idx = i;
		}
		return idx;
	}

	// selectionSort 选择排序,对向量区间[lo, hi)进行排序
	template <typename T>
	void Vector<T>::selectionSort(Rank lo, Rank hi)
	{
		...
		{
			hi--;
			Rank idx = selectMax(lo, n);
			swap(elem_[idx], elem_[hi]);
			n--;
		}
	}
}
