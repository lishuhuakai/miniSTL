...
#define ListNodePosi(T) ListNode<T>*
	...
	protected:
		ListNodePosi(T) search(T const& e, int n, ListNodePosi(T) it) const;	// 有序区间查找
	...
	public:
		// 可写访问接口
		iterator pushFront(T const& e);  // 将e当做首节点插入
		iterator pushBack(T const& e);   // 将e当做末节点插入
	...
	// pushFront 将e当做首节点插入
	template <typename T>
	typename List<T>::iterator List<T>::pushFront(T const& e) 
	{
		size_++;
	...
	// pushBack 将e当做末节点插入
	template <typename T>
	typename List<T>::iterator List<T>::pushBack(T const& e) 
	{
		size_++;
	...
	//*search 有序列表查找, 在有序列表内节点p(可能是trailer)的n个真前驱中,找到不大于e的最后着
	template <typename T>
